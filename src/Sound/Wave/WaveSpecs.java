...
public abstract class WaveSpecs {
    ...
    private double[] lSampleData;
    private double[] rSampleData;
    // bytes conversion
    protected byte[] convertedBytes;
    // bytes to string conversion
    protected String bytesToString;
    
    // all args constructor Sterio
    public WaveSpecs(int channels, long sampleRate, int bitsPerSample, 
            double[] lSampleData, double[] rSampleData, boolean isFloat) {
        wChannels = channels;
        dwSamplesPerSec = sampleRate;
        wBitsPerSample = bitsPerSample;
        this.lSampleData = lSampleData;
        this.rSampleData = rSampleData;
        wBlockAlign = (int)(wChannels * (wBitsPerSample / 8));
        dwAvgBytesPerSec = sampleRate * wBlockAlign;
        dwDataChunkSize = wBlockAlign * lSampleData.length;
        floatingPoint = isFloat;
    }
    
    // five args constructor Mono
    public WaveSpecs(int channels, long sampleRate, int bitsPerSample, 
            double[] sampleData, boolean isFloat) {
        this(channels, sampleRate, bitsPerSample, sampleData, SAMPLE_DATA, 
                isFloat);
    }
    ...
    public double[] getLSampleData() {
        return lSampleData;
    }

    public double[] getRSampleData() {
        return rSampleData;
    }
    ...
    public void setLSampleData(double[] lSampleData) {
        this.lSampleData = lSampleData;
    }

    public void setRSampleData(double[] rSampleData) {
        this.rSampleData = rSampleData;
    }

    public void setBytesToString(String bytesToString) {
