...
public class UndoDemo
{
   ...
   public UndoDemo()
   {
      ...
      /*
       * Add an action listener to the fully-undoable check box. Whenever an action is performed, create a new
       * UndoableEdit item that has methods to undo and redo the action. Add this edit to the undo manager, and update
      ...
      fullUndoCheckBox.addActionListener(new ActionListener()
      {
         public void actionPerformed(ActionEvent event)
         {
            ...
            // Save our current state in a final boolean so it is accessible in the inner class below
      ...
      slider.addChangeListener(new ChangeListener()
      {
         public void stateChanged(ChangeEvent event)
         {
            ...
            /*
             * We only want to record a change to the slider if the user is done adjusting it. Also, we need to make
             * sure we don't add a new event to the undo manager if we are currently changing the slider's value as a
             * result of undoing an action. Thus, we keep track of whether an undo/redo is in progress or not
             */
            if (!slider.getValueIsAdjusting())
            {
               ...
               if (!undoRedoInProgress)
               {
                  ...
                  // Save our old and new slider values in final int's that will be accessible in
                  ...
                  UndoableEdit undoableEdit = new AbstractUndoableEdit()
                  {
                     public void redo() throws javax.swing.undo.CannotRedoException
                     {
                        ...
                        // Flag that we are in a redo, then change the slider value to the new value
                        // If we don't do this, we will create a new undoable edit, which we don't want to do...
                        // This is because when we call "setValue" then the stateChanged event will be called again
                     ...
                     public void undo() throws javax.swing.undo.CannotUndoException
                     {
                        ...
                        // Flag that we are in an undo, then change the slider value to the new value
                        // If we don't do this, we will create a new undoable edit, which we don't want to do...
                        // This is because when we call "setValue" then the stateChanged event will be called again
                        undoRedoInProgress = true;
      ...
      /*
       * Add a window listener to our frame to quit the app if the main frame is closed.
       */
